...
var socket = io('http://localhost:8080',{"force new connection":true});
...
function getDevices(){
	console.log("ba me no get roi");
	$http.get('http://localhost:8080/devices').then(function(response){
...
function deleteDevice(index,devices) {
    ...
    $http.delete(url + '/devices/'+registrationId).then(function(response) {
...
$scope.showSendMessageDialog = function(ev,index) {
    ...
    $mdDialog.show(confirm).then(function(result) {
    	...
    	var registrationId = $scope.devices[index].registrationId;
      	...
      	$http.post(url + '/send', params).then(function(response) {
